...
public class GraphPanel extends JPanel {
    ...
    private static double maxScore = 3;

    public GraphPanel(double[] scores2) {
        this.scores = scores2;
    }
...
//    }
    ...
    private static void createAndShowGui() {
    	double minScore = 1.625;
    	int maxDataPoints = 100;
    	double maxVarience = .07;
        double[] scores = new double[maxDataPoints];
        
        double randomNum = (Math.random() * maxScore);
        if (randomNum < minScore) {
        	randomNum = minScore;
        }
        scores[0] = randomNum;
        System.out.printf("%.2f", scores[0]);
        System.out.println("\n");
        for (int i = 1; i < maxDataPoints; i++) {
        	double randomChange = ((Math.random() * maxScore) * maxVarience) - ((maxVarience/2) * maxScore);
            scores[i] = scores[i-1]+randomChange;
            ...
            if (scores[i - 1] == maxScore) {
        		scores[i] -= ((maxVarience/2) * maxScore);
